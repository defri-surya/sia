...
//            nomsg: 'No items',
        ...
        var g = {
            ...
            addData: function (data) { //parse data
                ...
                if (p.dataType == 'json') {
                    $.each(data.rows, function (i, row) {
                        ...
                        if(row['additionalAttr']){
                            if(row['additionalAttr']['name'] == 'tabindex'){
                               t.isTabIndex = true; 
                            }
                            jtr.attr(row['additionalAttr']['name'], row['additionalAttr']['value']);
                        }
                        /* akhir tambahan */
                ...
                $.ajax({
                    ...
                    success: function (data) {
                        ...
                        if(t.isTabIndex){
                            ...
                            $('tr[tabindex="1"].control-row:visible').addClass('trSelected').focus();
                        }
                        /* Akhir Tambahan */
                    },
            ...
            doSearch: function () {
                ...
                var querys = [];
                $('#'+ t.id + '_formModalFilter .querys').each(function () {
                    if ($(this).val() != '') {
...
//                p.qtypes = $('.search_popup input[name=querys]').attr('title');
                p.query = $('#'+ t.id + '_formModalFilter input[name=q]', g.sDiv).val();
                p.date_start = $('#'+ t.id + '_formModalFilter input[name=qdatestart]', g.sDiv).val();
                p.date_end = $('#'+ t.id + '_formModalFilter input[name=qdateend]', g.sDiv).val();
                p.num_start = $('#'+ t.id + '_formModalFilter input[name=qnumstart]', g.sDiv).val();
                p.num_end = $('#'+ t.id + '_formModalFilter input[name=qnumend]', g.sDiv).val();
                p.qtype = $('#'+ t.id + '_formModalFilter select[name=qtype]', g.sDiv).val();
                p.newp = 1;
                this.populate();
                /* ADDITIONAL ENDS HERE / AKHIR TAMBAHAN */
            },
        ...
        if (p.usepager) {
            ...
            $(g.bDiv).after(g.pDiv);
            var html = ' <div class="pGroup"> <div class="pFirst pButton"><span title="Pertama"></span></div><div class="pPrev pButton"><span title="Sebelumnya (ALT+<)"></span></div> </div> <div class="btnseparator"></div> <div class="pGroup"><span class="pcontrol">' + p.pagetext + ' <input title="Inputkan Halaman (ALT+H)" id="current_page" type="text" size="4" value="1" /> ' + p.outof + ' <span> 1 </span></span></div> <div class="btnseparator"></div> <div class="pGroup"> <div class="pNext pButton"><span title="Selanjutnya (ALT+>)"></span></div><div class="pLast pButton"><span title="Terakhir"></span></div> </div> <div class="btnseparator"></div> <div class="pGroup"> <div class="pReload pButton"><span title="Reload Grid (ALT+R)"></span></div> </div> <div class="btnseparator"></div> <div class="pGroup"><span class="pPageStat"></span><span id="total_data" style="display:none;"></span></div>';
            ...
            if (p.useRp) {
                ...
                $('.pDiv2', g.pDiv).prepend("<div class='pGroup'><select name='rp' title='Range per Halaman (ALT+P)'>" + opt + "</select></div> <div class='btnseparator'></div>");
            ...
            //add search button
            if (p.searchitems) {
                $('.pDiv2', g.pDiv).prepend("<div class='pGroup'> <div id='" + t.id + "_pSearch' class='pSearch pButton'><span title='Pencarian (ALT+C)'></span></div> </div>  <div class='btnseparator'></div>");
...
//                });
                ...
                for (var s = 0; s < sitems.length; s++) {
                    ...
                    colInput += '<div class="col-md-12 col-sm-12 col-xs-12">';
                    colInput += '<div class="form-group">';
                    if (type == 'text') {
                        colInput += '<label for="' + collName + '">' + collDisplay + '</label><input tabindex="'+(countTabIndex+1)+'" type="text" class="form-control querys querys_text" id="' + t.id + '_' + collName + '" placeholder="' + collDisplay + '" param="' + collName + '">';
                        countTabIndex = countTabIndex+1;
                    } else if (type == 'num') {
                        colInput += '<label for="' + collName + '">' + collDisplay + '</label><input tabindex="'+(countTabIndex+1)+'" id="' + t.id + '_' + collName + '" type="text" class="form-control querys querys_num_start col-md-2" param="' + collName + '" placeholder="Min"> <input tabindex="'+(countTabIndex+2)+'" id="' + t.id + '_' + collName + '" type="text" class="form-control querys querys_num_end" param="' + collName + '" placeholder="Max">';
                        countTabIndex = countTabIndex+2;
                    } else if (type == 'select') {
                        colInput += '<label for="' + collName + '">' + collDisplay + '</label>';
                        colInput += '<select tabindex="'+(countTabIndex+1)+'" class="form-control select2 querys querys_option" style="width: 100%" id="' + t.id + '_' + collName + '" param="' + collName + '">';
                        ...
                        colInput += '<input tabindex="'+(countTabIndex+1)+'" type="text" class="form-control hasDateRange querys querys_date" id="' + t.id + '_' + collName + '" param="' + collName + '">';
                ...
                $('body').append(
                        ...
                        '<div class="block_form" style="padding-right: 15px;">' +
                        '<div class="row">' + colInput +
                        '</div>' +
                        '</div></div>' +
                        '<div class="modal-footer">' +
                        '<button id="' + t.id + '_search" type="button" tabindex="'+(countTabIndex+1)+'" class="btn btn-primary btn_grid_filter"><i class="fa fa-search"></i>&nbsp;<u>C</u>ari</button>' +
                        '<button id="' + t.id + '_reset" type="button" class="btn btn-dark"><i class="fa fa-refresh"></i>&nbsp;<u>R</u>eset Pencarian</button>' +
                ...
                $("#"+ t.id + "_formModalFilter .select2").select2({
                    dropdownParent: $("#"+ t.id + "_formModalFilter")
                });
...
//                    g.doSearch();
//                });
                $("#" + t.id + "_reset").on('keydown click', function (e) {
...
//                        }
//                    });
                    $('#' + t.id + '_search').on('keydown click', function (e){
                        if(e.type == 'click' || e.keyCode == 13){
                            $('#'+ t.id + '_formModalFilter').modal('hide');
                            g.doSearch();
                        }
...
//            });
    ...
    $.fn.flexClearReload = function (p) { // function to clear and reload grid
        return this.each(function () {
            if (this.grid && this.p.url) {
                this.grid.doClear();
                // this.grid.populate();
...
function act_show(com, grid, urlaction) {
    ...
    if ($('.trSelected', grid).length > 0) {
        ...
        var conf = confirm('Anda yakin akan ' + title + ' ' + $('.trSelected', grid).length + ' data?\nData tidak dapat dikembalikan lagi!');
        if (conf == true) {
            var arr_id = [];
            var i = 0;
        ...
        alert('You haven\'t selected data!');
    }
}
