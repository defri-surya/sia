...
class Registration extends Backend_Controller {
    ...
    public function show() {
        ...
        $this->template->content("membership/registration_list_view", $data);
    ...
    public function get_data_member() {
        ...
        if ($response->status == 200) {
            ...
            foreach ($results as $row) {
                $data_id = array(
                    'id' => $row->member_id,
                    'text' => "(" . $row->member_code . ") " . $row->member_name
                );
                
                $entry = array('id' => json_encode($data_id), 'additionalClass' => 'control-row',
    ...
    public function get_code_account() {
        $res = $this->curl->get(URL_API . 'option/code', array(
            'table' => 'sys_member_product_saving',
            'field' => 'member_product_account_number'
        ));
        echo $res;
    }
    ...
    public function act_add() {
        ...
        $_POST['saving_obligation'] = convert_format_rp($_POST['saving_obligation']);
        ...
        if ($member_birthdate != NULL && $member_join_datetime != NULL) {
            ...
            $res = $this->curl->post(URL_API . 'membership/registration/act_add', $this->input->post());
            echo $res;
        } else {
            $res = array(
                'status' => 400,
    ...
    public function act_add_entrance_fee() {
        $res = $this->curl->post(URL_API . 'membership/entrance_fee/act_add', $this->input->post());
        echo $res;
    }
